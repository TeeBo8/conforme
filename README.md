...
![Tests](https://img.shields.io/badge/tests-97%20passing-2ea44f?logo=vitest&logoColor=white)
...
2. **The LLM only writes non-normative text**: a plain-language explanation of why the site processes data, and, when the user asks for it, a proposed description of their activity, read from their own website and shown for review. It never touches a clause.
...
4. **When the LLM cannot know, it is not asked.** An early version asked Claude to describe the company's activity from its name alone. An audit showed it was inventing businesses. The fix was a better input, not a better prompt: the activity is now proposed only from the real text of the user's homepage, and the model answers "none" when that text is not enough.
...
- **Site scan**: paste your URL and the homepage is read to pre-fill the form: contact details, SIRET, hosting provider, trackers (Google Analytics, Meta Pixel, YouTube embeds...), forms and payment. Only empty fields are filled; everything stays editable.
- Consistency checks: 14-digit SIRET, US host without a declared transfer outside the EU, cookies that require a consent banner (with a warning when trackers are detected without one).
...
## Fetching untrusted URLs safely

The site scan makes the server fetch a URL chosen by an anonymous visitor, the classic setup for SSRF. The guards, in [`src/server/scan`](src/server/scan):

- only `http`/`https` on ports 80/443, no credentials in the URL;
- **every resolved IP is checked at connection time** through Node's `lookup` hook (private, loopback, link-local and cloud-metadata ranges, IPv4-mapped IPv6...), which also defeats DNS rebinding;
- redirects are followed manually (3 max), each hop re-validated;
- 8 s timeout, 1.5 MB cap, HTML only; the page is never sent back to the browser, only the extracted suggestions;
- a per-IP rate limit, and the page text is passed to the model as delimited, untrusted data.

`localhost`, `127.0.0.1.nip.io`, `http://2130706433`, `169.254.169.254` and a redirect to `127.0.0.1` are all rejected. IP ranges and URL parsing are covered by [unit tests](src/server/scan/__tests__).
...
flowchart LR
  ...
  U[Site scan · URL] --> SC[Safe fetch · SSRF guards]
  SC --> A[Deterministic extraction<br/>contact · host · trackers]
  SC --> LA[Claude Haiku<br/>activity from real text]
  A --> F
  LA --> F
...
| [`src/server/scan`](src/server/scan) | Site scan: SSRF-safe fetch, extraction, activity proposal, rate limit |
| [`src/server/ai/claude.ts`](src/server/ai/claude.ts) | The only entry point to the LLM: timeout, logging, output clean-up |
| [`src/server/api/routers/document.ts`](src/server/api/routers/document.ts) | Generation and scan endpoints (tRPC) |
...
`ANTHROPIC_API_KEY` is optional: without it, documents are generated without the explanatory paragraph and the scan skips the activity proposal.

```bash
pnpm test    # 97 unit tests: legal templates, escaping, site analysis, SSRF guards
pnpm lint
pnpm build
```
